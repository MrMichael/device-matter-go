...
WORKDIR /
COPY --from=builder /device-matter-go/Attribution.txt /
COPY --from=builder /device-matter-go/LICENSE /
COPY --from=builder /device-matter-go/cmd /

EXPOSE 59920

ENTRYPOINT ["/device-matter"]
